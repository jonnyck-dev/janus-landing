...
// fetch(request) deja que Cloudflare rutee internamente al tunel via CNAME.
// Si el tunel (PC local) esta caido (530/502/521/522/1033), sirve mantenimiento.
...
const TUNNEL_DOWN = [530, 502, 521, 522, 1033];
...
export default {
  async fetch(request) {
    try {
      ...
      if (TUNNEL_DOWN.includes(response.status)) {
